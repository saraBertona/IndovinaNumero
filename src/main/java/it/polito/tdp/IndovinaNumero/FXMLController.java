...
public class FXMLController {
	...
	private Model model; //devo definirlo in EntryPoint
    ...
    void doNuovaPartita(ActionEvent event) {
    	//gestione di una nuova partita sarà in Model
    ...
    void doTentativo(ActionEvent event) {
    	...
    	//controllo 1 -> input numerico, va bene farlo qua
    	try {
    	...
    	try {
    		...
    		txtRisultato.setText(ip.getMessage());// ricorda
    		return;
    	...
    	if(risultato == 0) {
    		//HAI VINTO
    		txtRisultato.setText("HAI INDOVINATO CON " + model.getTentativiFatti() + " TENTATIVI");
    		hboxTentativi.setDisable(true);
    		return;
    	}
    	else if(risultato == -1){
